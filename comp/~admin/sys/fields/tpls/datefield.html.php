...
<div class="mx-formpanel">
	<form class="ajaxform" method="post" action="sys/fields/do/?Fields-<?php echo Template::$_tplval['actinfo']; ?>.html">
	  ...
	  <legend>时间“<?php echo Template::$_tplval['info']['livname']; ?>(<?php echo Template::$_tplval['info']['proname']; ?>)”</legend>
	  <table border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="300">
		    <label text="字段标签">字段标签</label>
			<input type="text" name="fieldlabel" dataType="Require" itip="请填写字段标签" />
			<input name="xtype" type="hidden" value="datefield" class="noload"/>
		...
		<tr>
          <td width="300">
			...
			<input type="text" name="listwidth" dataType="Number" value="120" itip="列表宽只能填写数字" />
		  </td>
        </tr>
		<tr>
          <td width="300">
		...
		<tr>
          <td width="300">
		    <label text='默认值(如果是当前时间请填写"NOW")'>默认值(如果是当前时间请填写"NOW")</label>
			<input type="text" name="defval" />
		  </td>
        </tr>
		<tr>
          <td width="300">
		    <label text="时间格式(年:yyyy,月:MM,日:dd,时:HH,分:mm,秒:ss)">时间格式(年:yyyy,月:MM,日:dd,时:HH,分:mm,秒:ss)</label>
			<input type="text" name="datefmt" dataType="Require" itip="请填写时间格式" />
		  </td>
        </tr>
		<tr>
          <td width="300">
